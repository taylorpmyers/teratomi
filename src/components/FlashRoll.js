...
class FlashRoll extends React.Component {
  render() {
              ...
              <div className="w-md flex">
                ...
                <div className="ml-2 w-full flex-wrap leading-tight border-t-2 border-b-2 border-teal-600 h-full">
                  <div className="ml-2 mt-1 w-md">
                    <p className="text-sm text-white mb-0">Flash Fic / {post.frontmatter.date}</p>
...
export default props => (
  <StaticQuery
    query={graphql`
      query FlashRollQuery {
        allMarkdownRemark(
          sort: { order: DESC, fields: [frontmatter___date] }
          filter: { frontmatter: { templateKey: { eq: "flashfic-post" } } }
        ) {
          edges {
            node {
    ...
    `}
    render={(data, count) => <FlashRoll data={data} count={count} num={props.num} isResponsive={props.isResponsive} />}
  />
)
